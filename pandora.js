...
var at = 'wnb300BmFVHfJjjFZo/ISxvXK+OFmuiaA'; //authetication token

function getSomeSongs(stationId, startIndex, cb) { //getsongs from station?
	...
	request(stationUrl, function(error, response, body) { //send request to stationurl
		if (error || response.statusCode !== 200) //if not succesful, pandora done got the boy :(
		...
		if (html.get('//div[@class="no_more"]')) //i'm guessing this is the div tag in the pandora 
			cb(null, {songs: songs, hasMore: false}); //this is where hasmore variable comes from. if div class not no more, then hasMOre is false
		else
			cb(null, {songs: songs, hasMore: true});
	});
};

getSomeStations = function(username, stations, cb) { //get some stations
	stations = stations || [];

	var stationsUrl = 'http://www.pandora.com/content/stations?startIndex=' + stations.length + '&webname=' + username; //get station

	var jar = request.jar(); // stores cookie
